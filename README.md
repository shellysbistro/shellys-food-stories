# Shelly's Food Stories Dashboard

An interactive, responsive prototype for the St. James-Assiniboia School Division Universal Nutrition Program.

## Open the dashboard

Open `index.html` in a browser, or run any local static server in this folder.

No package installation or build step is required.

## Included

- 45-item source menu inventory
- 45 individual, menu-specific food stories — including separate chapters for every soup and salad
- Six source-linked, interactive food-history comic chapters
- 45 unique, menu-specific storybook food images — one generated and visually checked for every named item
- 45 unique, menu-specific food character images with matching speech-bubble clues
- A prominent five-stop field, farm or water-to-school traceability trail for every item
- Original food-character illustrations and four-panel history illustrations
- Clue challenges, story stamps and saved passport progress
- K–3, Grades 4–6 and Grades 7–12 reading levels
- Search and cultural/geographic filters
- Ingredient farm-to-school journeys
- Interactive food map
- Teacher prompt generator and printable lesson view
- Discussion-card deck
- Five-layer recommended school package
- 25-deliverable coverage overview

## Content note

This is an educational concept based on the supplied Shelly's Food Stories brief. Item ingredients and allergen designations must be checked against the current approved Shelly's menu. The traceability trails explain likely food-system routes; actual farms, processors, countries of origin, supplier names and lot codes must be verified from Shelly's receiving and preparation records.

The comic chapters currently link to source material from Parks Canada, the Louis Riel Institute/Virtual Museum of Métis History and Culture, Smithsonian, Turismo Roma and Université Laval's IREPI.

## Brand note

The supplied Shelly's Bistro logo is used exactly as provided in the interface and favicon. The surrounding visual system uses a warm, botanical-inspired school-food direction. Food imagery is original educational illustration and is explicitly labelled as such in the interface; supplement it with real Shelly's food photography when approved photography is available.
